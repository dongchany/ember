...
// =============================================================================
// Zero-Centered RMSNorm Kernels (for Qwen3.5 gated attention Q/K norm)
// =============================================================================

template<int BLOCK_SIZE>
__global__ void zero_centered_rms_norm_kernel_f16(
    half* output,
    const half* input,
    const half* weight,
    int hidden_size,
    ...
    __shared__ float s_warp_sum[32];
    __shared__ float s_warp_sq[32];

    float sum = 0.0f;
    for (int i = tid; i < hidden_size; i += BLOCK_SIZE) {
        sum += __half2float(row_input[i]);
    }
    sum = warp_reduce_sum_rmsnorm(sum);
    if (lane_id == 0) {
    ...
    const float mean = s_warp_sum[0] / static_cast<float>(hidden_size);

    float sum_sq_centered = 0.0f;
    for (int i = tid; i < hidden_size; i += BLOCK_SIZE) {
        const float centered = __half2float(row_input[i]) - mean;
        sum_sq_centered += centered * centered;
    }
    sum_sq_centered = warp_reduce_sum_rmsnorm(sum_sq_centered);
    if (lane_id == 0) {
        s_warp_sq[warp_id] = sum_sq_centered;
    }
    __syncthreads();

    if (warp_id == 0) {
        float v = (lane_id < num_warps) ? s_warp_sq[lane_id] : 0.0f;
        v = warp_reduce_sum_rmsnorm(v);
        if (lane_id == 0) {
            s_warp_sq[0] = v;
        }
    ...
    const float rms_inv = rsqrtf(s_warp_sq[0] / static_cast<float>(hidden_size) + eps);
    for (int i = tid; i < hidden_size; i += BLOCK_SIZE) {
        const float centered = __half2float(row_input[i]) - mean;
        const float w = __half2float(weight[i]);
        row_output[i] = __float2half(centered * rms_inv * w);
    }
}

template<int BLOCK_SIZE>
__global__ void zero_centered_rms_norm_kernel_bf16(
    __nv_bfloat16* output,
    const __nv_bfloat16* input,
    const __nv_bfloat16* weight,
    int hidden_size,
    ...
    float sum_sq_centered = 0.0f;
    for (int i = tid; i < hidden_size; i += BLOCK_SIZE) {
        const float centered = __bfloat162float(row_input[i]) - mean;
        sum_sq_centered += centered * centered;
    }
    sum_sq_centered = warp_reduce_sum_rmsnorm(sum_sq_centered);
    if (lane_id == 0) {
    ...
    for (int i = tid; i < hidden_size; i += BLOCK_SIZE) {
        ...
        row_output[i] = __float2bfloat16_rn(centered * rms_inv * w);
    }
}
...
void zero_centered_rms_norm_f16(
    ...
    const int num_rows = batch_size * seq_len;
    if (hidden_size <= 1024) {
        zero_centered_rms_norm_kernel_f16<256><<<num_rows, 256, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    } else if (hidden_size <= 4096) {
        zero_centered_rms_norm_kernel_f16<512><<<num_rows, 512, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    } else {
        zero_centered_rms_norm_kernel_f16<1024><<<num_rows, 1024, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    }
}
...
void zero_centered_rms_norm_bf16(
    ...
    const int num_rows = batch_size * seq_len;
    if (hidden_size <= 1024) {
        zero_centered_rms_norm_kernel_bf16<256><<<num_rows, 256, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    } else if (hidden_size <= 4096) {
        zero_centered_rms_norm_kernel_bf16<512><<<num_rows, 512, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    } else {
        zero_centered_rms_norm_kernel_bf16<1024><<<num_rows, 1024, 0, stream>>>(
            output, input, weight, hidden_size, eps);
    }
}
